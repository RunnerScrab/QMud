                                                     ...
                                                     const bool allowLayoutBuild) const
{
...

	{
		...
		for (const ScrollPane &pane : panes)
		{
			...
			// Cap the top pane at the cut by clamping the value, not the maximum (which the renderer scales against)
...
bool WorldView::isAtBufferEnd() const
{
	...
	if (!bar)
		return true;
	// While split, the top pane's end is the cut at the bottom pane's top edge, not bar->maximum()
